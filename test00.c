...
int Child(char *arg) 
{
  ...
  if (time > 1000000) {
    printf("Child%d(): Sleep bad: %d %d\n", me, time, ABS(10000000, time));
  }
  else {
    printf("Child%d(): Sleep done at time %d\n", me, end);
  }
  Terminate(1);
...
int start4(char *arg)
{
  ...
  printf("start4(): Start 5 children who all sleep for 10 seconds. Upon\n");
  printf("          waking up, each child checks if its sleep time was at\n");
  printf("          least 10 seconds.\n");
